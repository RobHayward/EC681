...
\item Thomas Edison vs Nicola Tesla
...
\item Gaming
\item GPUs
\item BTC and solving the hash
\item AI, NN, GPUs and Nvidia
\end{itemize}
\end{frame}

\begin{frame}{Nvidia vs BTC}
<<Nvidia, echo = FALSE, message=FALSE, warning=FALSE, fig.height=4.6, cache=TRUE>>=
require(quantmod)
NVDA <- getSymbols('NVDA', auto.assign = FALSE)
BTC <- getSymbols('BTC-USD', auto.assign = FALSE)
da <- merge(Cl(NVDA), Cl(BTC))
...
\item Hash problem adjusted to take 10 mins
\end{itemize}
\end{frame}
...
\item AI is now a more important energy focus
\end{itemize}
\end{frame}
...
\begin{frame}{Tether}
Total Assets \$185bn (Q3 2025)
\begin{itemize}[<+-| alert@+>]
\item US Treasury bills (\$135bn)
\item Cash and short-term deposits
\item Physical gold
\item Other (secured loans, corporate bonds)
\end{itemize}
\pause
\href{https://tether.to/en/transparency/?tab=reports}{Assets}
\vspace{1.5cm}
Profits were \$10bn year-to-Q3 2025
\end{frame}

\begin{frame}{Assets}
\includegraphics[scale=1.0]{tetherassets}
\end{frame}

\begin{frame}{Links to conventional finance}
\begin{itemize}[<+-| alert@+>]
\item Crypto world is the \emph{wild west} with high margins and low regulation
\item Private and narrow banking
\item Some Crypto players want legitimacy/respectability (but not all)
\item Regulators are poised (Gary Gensler SEC vs Trump)
\item Link between conventional and Crypto world is traditionally murky, risky and expensive
\end{itemize}
\end{frame}
...
Adjustments to deal with scale
\begin{itemize}[<+-| alert@+>]
\pause
...
\begin{frame}{Howey Test}
The Howey test of \emph{investment product}
\pause
\begin{block}{}
\begin{itemize}
\item There is an investment of money or other value
\item A common enterprise
\item With a reasonable expectation of profit
\item With those profits primarily coming from the efforts of others
\end{itemize}
\end{block}
\end{frame}
