...
\begin{itemize}[<+-| alert@+>]
\pause
\item We do not know how much dividends will be paid
...
\end{block}
\vspace{1cm}
$D_t$ = dividend, $R$ = rate of return or yield, t = time in years. 
\end{frame}
...
\item Average PE ratio for the S\&P 500 for the last 100 years is around 15
\end{itemize}
\end{frame}
...
\begin{itemize}[<+-| alert@+>]
\pause
\item Bottom up:  this is the corporate finance way of understanding the performance of individual companies and amalgamating that to get an overall view of the market
...
plot(da$Date, da$CAPE, type = 'l', main = "CAPE 1871 to 2025", 
     ylab = "CAPE", xlab = "Year")
...
\hline
Russell 2000 & 35.32 & 25.0  \\
NASDAQ 100 & 33.15 & 31.7 \\
S\&P 500 & 25.47 & 24.4 \\
\end{tabular}
\end{center}
\end{table}
...
\begin{frame}{Tobin's Q over time}
<<tobinq, echo = FALSE, warning=FALSE, message=FALSE, fig.height=4.5, fig.cap= "Tobin's Q">>=
