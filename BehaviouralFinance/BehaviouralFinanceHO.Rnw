...
\item Example, quite, socially-insecure, star-wars loving, boy and computer science (but small course)
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{The portfolio}
For the assignment
\begin{itemize}[<+-| alert@+>]
\pause
\item See if there is a gradual reaction to news
\begin{itemize}
\item do assets continue to drift after the announcement? 
\item is some information more gradually absorbed than others?
\end{itemize}
\item What can identify a reversal?
\begin{itemize}
\item too optimistic or too pessimistic
\item different for different assets?
\end{itemize}
\end{itemize}
\end{frame}
